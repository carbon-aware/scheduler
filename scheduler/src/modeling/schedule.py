...
from src.forecasting.conversion import convert_power_zone_to_cloud_zones, PowerZone
from src.forecasting.fetch import fetch_forecast
from src.types.schedule import ScheduleRequest, ScheduleResponse, TimeRange, ScheduleOption, CarbonSavings, CloudZone
import pandas as pd
...
async def compute_schedule(request: ScheduleRequest) -> ScheduleResponse:
    ...
    if rolled_forecast_df.empty:
        raise ValueError("No valid forecast data available for the requested time windows and zones")
    ...
    for i in range(min(num_options, len(options_df))):
        ...
        top_n_options.append(ScheduleOption(
            time=option['time'],
            zone=option['zone'],
            co2_intensity=option['co2_intensity']
        ))
    
    # Calculate carbon savings
    carbon_savings = CarbonSavings(
        vs_worst_case=float((worst_case.co2_intensity - ideal.co2_intensity) / worst_case.co2_intensity * 100),
        vs_naive_case=float((naive_case.co2_intensity - ideal.co2_intensity) / naive_case.co2_intensity * 100),
        vs_median_case=float((median_case.co2_intensity - ideal.co2_intensity) / median_case.co2_intensity * 100)
    )
...
def _rollup_forecast_to_job_duration(forecast_df: pd.DataFrame, duration: timedelta) -> pd.DataFrame:
...
def _trim_forecast_to_windows(forecast_df: pd.DataFrame, windows: list[TimeRange], duration: timedelta) -> pd.DataFrame:
    ...
    for window in windows:
        ...
        window_mask = (forecast_df['point_time'] >= window.start) & (forecast_df['point_time'] <= window_end_with_duration)
